...
void main() {
  ...
  late StopMirroring useCase;

  setUp(() {
    mockScreenMirroringRepository = MockScreenMirroringRepository();
    useCase = StopMirroring(mockScreenMirroringRepository);
  });

  test('should call stopMirroring from repository', () async {
    // arrange
    ...
    const tParams = StopMirroringParams(tDevice);
    when(() => mockScreenMirroringRepository.stopMirroring(tDevice.adbSerial))
        .thenAnswer((_) async => right(null));
    // act
    await useCase(tParams);
    // assert
    verify(() => mockScreenMirroringRepository
        .stopMirroring(tParams.deviceInfo.adbSerial));
  });
}
